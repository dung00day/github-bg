...
from GUI.camera_final import Ui_MainWindow
...
class MyApplication(QMainWindow):
    ...
    def receiveSignalSfc(self):
        ...
        while True:
            ...
            if self.result_s4_190 == s4_190_OK:
                if signal_sfc == sfc_OK:
                    print('sfc trả về OK, chờ công đoạn scan tiếp...')
                    logger.info(f'result from sfc: {sfc_OK}')
                    self.changeStatus(status_pass)
                    break
                elif signal_sfc == sfc_NG:
                    print('SFC Fail')
                    logger.error(f'result from sfc: {sfc_NG}')
                    self.changeStatus(status_ng_sfc)
                    self.machine1.send_data(sfc_NG)
                    break
            elif (self.result_s4_190 == s4_190_NG) | (self.result_s4_190 == s4_190_NG_cam):
                self.machine1.send_data(sfc_NG)
                print('Gửi NG cho máy 1')
                break
    ...
    def receiveSignalMachine1(self):
        ...
        while True:
            ...
            if signal_machine1 == plc_scan:
                print('Tín hiệu scan từ machine 1, gửi tín hiệu scan cho machine 4')
                ...
                #     quét qr thành công
                    ...
                    print('Chờ kết quả máy 4......')
                    self.waitResultMachine4()
                    break
