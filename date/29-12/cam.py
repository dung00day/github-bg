...
class MyApplication(QMainWindow):
    def __init__(self):
        ...
        self.ui.result.setStyleSheet("border: 1px solid rgb(85, 0, 0);color:  rgb(30, 30, "
                                     "30); background-color: rgb(221, 221, 110);")
        self.config = []
        self.configValue()
    ...
    def Camera(self):
        while True:
            ...
            if self.res:
                ...
                # self.thresh = cv2.adaptiveThreshold(blurred_img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY,
                #                                     11, 2)
                #
        ...
        while True:
            if self.flag_scan and self.res:
                ...
                if self.code_scan is None:
                    ...
                    print(f'--------{self.count}----------')
                    # self.ui.result.setText('PASS')
                    # self.ui.result.setStyleSheet("background-color: rgb(69,208, 102)")
                    # self.ui.result.setStyleSheet("border: 1px solid rgb(85, 0, 0); background-color: rgb(69,208, 102);")
                    # self.ui.frame.update()
                    if (self.count % 20) == 0:
                        print('update')
