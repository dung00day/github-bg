...
class MyApplication(QMainWindow):
    def __init__(self):
        ...
        self.serial_plc = ReadSerial(self.config[0], self.config[5])
        self.sfc_send = ReadSerial(self.config[1], self.config[6])
        self.sfc_receive = ReadSerial(self.config[2], self.config[6])
        self.machine2 = ReadSerial(self.config[3], self.config[6])
        self.machine3 = ReadSerial(self.config[4], self.config[6])
    ...
    def receiveSignalSfc(self):
        ...
        print('Xử lý kết quả để gửi cho plc: ', result_machine2)
