...
def scan_qr_code(image_path, threshold, count_pass=0):
    ...
    #             data = read_WeChatQRCode(cleaned_image, thresholded_image, erode, dilation)
    #             if data is not None:
    #                 print(f'thresh (no inv): {threshold}, img: {image_path}')
    #                 count_pass += 1
    #                 break
    #             else:
    #                 i += 1
...
# for image_file in image_files:
    ...
    # data = scan_qr_code(image_path, 60)
    # count += 1
    # if data is not None:
    #     count_pass += 1

image_path = r'D:\SD_CODE\images\image-sn\1704683759.33116.png'
# data = scan_qr_code(image_path, 185)
for i in range(0,255):
    data = scan_qr_code(image_path, i)
    if data is not None:
        print(i)
